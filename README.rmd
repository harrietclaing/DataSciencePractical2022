...
First, write a function to read-in multiple csv sheets.
```{r}
...
```{r}
NetflixDatadf <- NetflixData %>% group_by(release_year) %>% summarise_at(vars(imdb_score), ~mean(.))
NetflixDatadf %>% ggplot() + geom_col(aes(x=release_year, y=imdb_score), stat='identity', na.rm=T, scale) + theme_minimal()
```

Run time vs imdb scores?
```{r}
Netflixdf %>% ggplot() + geom_bar(aes(x=runtime, y=imdb_score), stat='identity')
```
